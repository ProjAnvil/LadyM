...
func (s *SQLiteStore) VectorSearch(queryVec []float32, topK int) []SearchHit {
	return s.vectorIndex.Search(queryVec, topK)
}
...
// TryAcquireIndexLock takes the cross-process code-index lock on
// <db>.index.lock and returns the release function. Contention fails fast
// with ErrIndexLockHeld — callers do not queue.
...
func (s *SQLiteStore) InvalidateEdge(edgeID string, t float64) error {
	_, err := s.db.Exec("UPDATE edges SET valid_to = ? WHERE id = ?", t, edgeID)
	return err
}
...
func (s *SQLiteStore) CountCodeSymbols() (int, error) {
	var n int
	err := s.db.QueryRow("SELECT COUNT(*) FROM code_symbols").Scan(&n)
	return n, err
}
