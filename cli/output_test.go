...
// Pure output-formatting and flag-wiring tests: no store, no engine, so they
// run in both editions. The store/engine-backed serve tests stay in
// cli_test.go (personal edition only).
...
// TestServeHTTPFlagRegistered covers the --http flag wiring on serveCmd.
func TestServeHTTPFlagRegistered(t *testing.T) {
	cmd := serveCmd()
	if cmd.Flags().Lookup("http") == nil {
		t.Fatal("serveCmd missing --http flag")
	}
}
