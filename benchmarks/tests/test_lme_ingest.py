...
from longmemeval import ingest
from lme_fixtures import make_mini_instance
...
def test_ingest_raw_auto_rebuilds_on_stale_db_probe_failure(tmp_path):
    """A stale DB the engine can't reopen auto-rebuilds (no --force needed).

    Simulates switching the embedding provider (e.g. hashing→ollama): probing the
    existing DB via _count_memories raises (over MCP this surfaces as a
    LadymError — the Go counterpart of the Python harness's
    EmbeddingDimensionMismatch). ingest must treat that as stale → rebuild.
    """
    from ladym_wrapper import LadymError
    ...
    def factory(**kw):
        calls["n"] += 1
        if calls["n"] == 1:
            # probing the stale DB raises (engine reopen fails over MCP)
            raise LadymError("embedding dimension mismatch: 256 vs 1024")
