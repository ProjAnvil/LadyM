...
VENDOR_DIR = (
    pathlib.Path(importlib.import_module("longmemeval.upstream_eval").__file__).parent
)
...
# CLI tools invoked via subprocess by the evaluate wrapper; never imported.
...
def test_importable_modules_import():
    for name in IMPORTABLE_MODULES:
        importlib.import_module(f"longmemeval.upstream_eval.{name}")
