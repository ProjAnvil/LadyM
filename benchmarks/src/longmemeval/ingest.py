...
from ._runtime import LadymError, make_engine as _default_engine_factory
    ...
    if not force:
        ...
        if cfg.variant == "raw" and db_path.exists():
            try:
                ...
                # Stale DB the Go engine can't reopen (e.g. built under a
                # different embedding provider/dim). Over MCP we can't
                # distinguish the exact cause, so any probe failure on an
                # existing DB is treated as stale: fall through to unlink +
                # rebuild so a provider switch doesn't require --force-ingest.
                pass
    if db_path.exists():
